...
def create_client(args,client:typing.Union[None,mqtt.Client]=None):
...
def main():
    ...
    if not os.path.isabs(os.path.dirname(args.private_key_file)):
        args.private_key_file = os.path.join(os.path.dirname(os.path.abspath(__file__)),args.private_key_file)

    if not os.path.isabs(os.path.dirname(args.ca_certs)):
        args.ca_certs = os.path.join(os.path.dirname(os.path.abspath(__file__)),args.ca_certs)

    # Create the MQTT client and connect to Cloud IoT.
    jwt_timestamp = datetime.datetime.utcnow()
    jwt_timeout_seconds = args.jwt_timeout_minutes * 60.0
    ...
    for _ in range(args.num_messages):
        ...
        if (jwt_timestamp - datetime.datetime.utcnow()).seconds >= jwt_timeout_seconds:
            ...
            client.connect(args.mqtt_bridge_hostname, args.mqtt_bridge_port)
            client.loop_start()
        pub_rc=client.publish(mqtt_telemetry_topic, payload, qos=1)
        pub_rc.wait_for_publish()
